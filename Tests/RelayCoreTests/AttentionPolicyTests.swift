...
struct AttentionPolicyTests {
    ...
    @Test
    func testAllStatusesStayInsideNotchRelayInsteadOfPostingSystemBanners() {
        for status in RelayStatus.allCases {
            #expect(!AttentionPolicy.decision(for: status, focusModeEnabled: false).shouldNotify)
        }
    }
}
